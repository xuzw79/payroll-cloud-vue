...
api.put("/ses/company-setting", async (c) => {
  ...
  const settings = await prisma.companySetting.upsert({
    ...
    update: {
      ...
      invoiceBankCode: nullableText(body.invoiceBankCode),
      invoiceBankBranch: nullableText(body.invoiceBankBranch),
      invoiceBankBranchCode: nullableText(body.invoiceBankBranchCode),
...
api.get("/ses/invoices/:id/pdf", async (c) => {
  ...
  const pdf = await createInvoicePdf({
    ...
    bankCode: settings.invoiceBankCode,
    bankBranch: settings.invoiceBankBranch,
    bankBranchCode: settings.invoiceBankBranchCode,
  ...
  try {
    const employee = await prisma.employee.create({
      data: {
        ...
        hireDate: nullableText(body.hireDate),
        retirementDate: nullableText(body.retirementDate),
        employmentStatus: ["ACTIVE", "LEAVE", "RETIRED"].includes(String(body.employmentStatus || "")) ? String(body.employmentStatus) : "ACTIVE",
        employmentType: ["REGULAR", "CONTRACT", "SOLE_PROPRIETOR", "OTHER"].includes(String(body.employmentType || "")) ? String(body.employmentType) : "REGULAR",
        email: nullableText(body.email),
        ...
        overtimeHourlyAmount: Number(body.overtimeHourlyAmount || 0),
